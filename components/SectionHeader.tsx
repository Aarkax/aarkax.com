...
export function SectionHeader({ eyebrow, title, description, align = "left", tone = "dark", className }: SectionHeaderProps) {
  return (
    <div className={cn("max-w-3xl", align === "center" && "mx-auto text-center", className)}>
      {eyebrow ? (
        <p className="mb-4 font-mono text-xs uppercase tracking-[0.18em] text-amber">{eyebrow}</p>
      ) : null}
      <h2
        className={cn(
      ...
      {description ? (
        <p className={cn("mt-5 text-base leading-8 sm:text-lg", tone === "light" ? "text-ivory/70" : "text-body")}>
          {description}
        </p>
      ) : null}
    </div>
